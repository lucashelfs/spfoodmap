...
6. Click on a country stores it as `selectedFeature` and calls `renderPanel()`, which rebuilds the `#panel` DOM: localized country name as heading, one `.restaurant-card` per restaurant, or an empty-state message if none curated. The clicked path's border is also highlighted (`#1a1a1a`, 2px, scaled inversely with zoom like the default stroke) and `raise()`d above its neighbors so the outline isn't clipped; the previous selection's border resets to default.
